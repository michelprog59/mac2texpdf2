...
\cfoot{ }
\rfoot{page \thepage}
\lfoot{Généré le \DTMnow}
\renewcommand{\headrulewidth}{0.0pt}
\renewcommand{\footrulewidth}{0.5pt}
\title{Résultats de Maxima vers LaTeX/PDF}
...
"""
    ...
    for line in lines:
        l = line.strip()
        if "ID:" in l: 
            body.append(f"\\subsection*{{Calcul {l.split(':')[-1].strip()}}}")
        elif "CMD:" in l:
            c = l.split('CMD:')[-1].strip()
            # Using listings to allow automatic line wrapping
            body.append(r"\textbf{Commande :}")
...
class YamwiApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Editeur Maxima vers LaTeX et PDF - v2.0")
        self.root.geometry("600x750")
        self.pdf_generated_path = None
        self.tmp_dir = None

        # Zone de saisie
        tk.Label(root, text="Saisir ou coller les commandes Maxima :", font=("Arial", 10, "bold")).pack(pady=5)
        ...
        opt_frame = tk.LabelFrame(root, text=" Options de mise en page ", padx=10, pady=10)
        opt_frame.pack(pady=10, fill=tk.X, padx=10)

        tk.Label(opt_frame, text="Taille de la police :").grid(row=0, column=0)
        self.font_size = tk.StringVar(value="10pt")
        tk.OptionMenu(opt_frame, self.font_size, "10pt", "11pt", "12pt").grid(row=0, column=1, padx=10)

        tk.Label(opt_frame, text="Echelle des graphiques (0.1-1.0) :").grid(row=0, column=2)
        ...
        # Bouton Générer
        self.btn_gen = tk.Button(root, text="GENERER PDF", command=self.process, bg="lightblue", font=("Arial", 10, "bold"))
        self.btn_gen.pack(pady=10, ipady=5, fill=tk.X, padx=50)

        # Actions après génération
        action_frame = tk.Frame(root)
        action_frame.pack(pady=5)
        self.btn_view = tk.Button(action_frame, text="Voir PDF", command=self.view_pdf, state=tk.DISABLED)
        self.btn_view.pack(side=tk.LEFT, padx=5)
        
        self.btn_save = tk.Button(action_frame, text="Exporter dossier (TeX + Images + PDF)...", 
                                 command=self.save_all, state=tk.DISABLED, bg="#d1ffcf")
        self.btn_save.pack(side=tk.LEFT, padx=5)

        tk.Button(root, text="Quitter", command=root.quit, bg="orange").pack(pady=15)
    ...
    def process(self):
        ...
        if not cmds:
            messagebox.showerror("Erreur", "La zone de texte est vide.")
            return

        self.tmp_dir = tempfile.mkdtemp()
        tex_path_0 = os.path.join(self.tmp_dir, "out.tex")
        tex_path = re.sub (r'\\', '/', tex_path_0)
        drv_path_0 = os.path.join(self.tmp_dir, "drv.mac")
        drv_path = re.sub (r'\\', '/', drv_path_0)

        try:
            print ('mac2texpdf2: tex_path = "%s", drv_path = "%s"' % (tex_path, drv_path))
            print ('mac2texpdf2: now attempt to write Maxima script to drv_path.')

            with open(drv_path, 'w', encoding='utf-8') as f:
                f.write(generer_script_maxima(cmds, tex_path, self.tmp_dir))
            
            if subprocess._mswindows:
                maxima_command = 'maxima.bat'
            else:
                maxima_command = 'maxima'

            print ('mac2texpdf2: now attempt to execute Maxima as: %s' % (maxima_command))
            # subprocess.run([maxima_command, '--very-quiet', '-b', drv_path], capture_output=True)
            subprocess.run([maxima_command, '-b', drv_path], capture_output=True)
            
            if os.path.exists(tex_path):
                print ('mac2texpdf2: Maxima created TeX output %s' % (tex_path))
            else:
                print ('mac2texpdf2: hmm, looks like Maxima failed to create TeX output %s' % (tex_path))

            if finaliser_tex(tex_path, self.font_size.get(), self.img_scale.get()):
                print ('mac2texpdf2: now attempt to execute pdflatex.')
                ...
                if os.path.exists(pdf_path):
                    ...
                    messagebox.showerror("Erreur", "Erreur lors de la compilation LaTeX.")
            else:
                messagebox.showerror("Erreur", "Maxima n'a pas pu traiter les commandes.")
    ...
    def save_all(self):
        ...
        full_dest_path = filedialog.asksaveasfilename(
            title="Choisir le nom du dossier d'exportation",
        ...
        if full_dest_path:
            ...
            try:
                ...
                messagebox.showinfo("Exportation", f"Dossier créé : {export_dir}\nFichiers : PDF, TeX et {count_img} image(s).")
            except Exception as e:
